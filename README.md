...
DisplayPlacer 是一个简单实用的显示器布局管理脚本，专为 macOS 用户设计。它主要解决了使用雷电集线器连接多个相同型号显示器时，系统无法正确识别和保持显示器配置的问题。

这个脚本源于个人需求：每次插入雷电4集线器后，两个相同型号的显示器会混淆，导致旋转状态和位置设置错误，需要手动重新调整。

## 屏幕截图

![DisplayPlacer 屏幕截图](screenCapture.jpg)
...
4. 提示您检查当前布局是否正确
5. 如果布局正确，您可以选择退出脚本
6. 如果布局不正确，脚本会交换两个外部显示器的配置
7. 应用配置后，再次检查布局是否正确
8. 如果配置正确，您可以选择保存此配置以便将来使用
